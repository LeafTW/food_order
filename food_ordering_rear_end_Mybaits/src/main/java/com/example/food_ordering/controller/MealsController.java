...
public class MealsController {
    ...
    public ResponseEntity<Integer> updateCartEntityById(@PathVariable Integer quantity, @PathVariable Long id) {
        mealsService.updateCartEntityById(quantity, id);
        return ResponseEntity.ok(null);
    }

    @GetMapping("/getMealsAllWithItems")
    public ResponseEntity<List<MealsItemDTO>> getMealsAllWithItems(){
        List<MealsItemDTO> mealsAllWithItem = mealsService.findMealsAllWithItem();
        return ResponseEntity.ok(mealsAllWithItem);
    }

    /**
     * 更新Item至MySql
     */
    @PostMapping("/updateItemShow")
    public ResponseEntity<Boolean> updateItemShow(@RequestBody UpdateItemRequest show){
        try {
            String key = "scheduledData:" + "30";
            mealsService.savedDataToRedis(key,show);
            return ResponseEntity.ok(true);
        }
        catch (Exception e){
